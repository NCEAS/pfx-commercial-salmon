...
To understand the how altering catch diversification affects individual revenue and revenue variability, we focused on four salmon gillnet and two purse seine salmon fisheries where some catch diversification exists (Table 1; Figs A3-A4). The same data filtering steps described in [@anderson_benefits] were applied to individual fish ticket data for years 1975-2016. Trip level catch and revenue data were aggregated to annual summaries for each individual permit holder. We restricted our analysis to only include permit holders who participated in a single fishery in a year; to avoid complications in modeling switching, pairs of years where individuals changed strategies were not included. Effective catch diversity was used as a proxy for diversification, calculated as the inverse of Simpson's diversity [@jost2006a; @anderson_benefits; @kasperski2013], with each species' contribution weighted by revenue. Effective catch diversity values near 1.0 indicate that an individual's revenue is dominated by a single species, whereas an effective catch diversity of 3.0 would indicate that three species contribute equally to revenue. Briefly, we implemented a Bayesian time series model with group-level predictors and individual covariates (diversity of catch, days fished) to simultaneously affect the mean and variance of revenue for individual fishers [@anderson_benefits]. In addition to using a longer time series (12 more years of data), the major difference in this modeling is allowing parameters associated with diversification to vary through time. This model is described in two parts below.

Following [@anderson_benefits], we used a class of regression models to simultaneously estimate effects of catch diversification on an individuals' mean annual revenue and variability in annual revenue. The natural log of revenue for individual *i* in year *t* is $${ \log  \left( { R }_{ i,t } \right)  } ={ B }_{ 0,j,t }+{ B }_{ 1,j,t }\cdot \Delta S_{ i,t }+{ B }_{ 2,j,t }\cdot \left( \Delta S_{ i,t }\cdot { S }_{ i,t-1 } \right) + { B }_{ 3,j }\cdot \Delta D_{ i,t }+\log  \left( { R }_{ i,t-1 } \right)$$ where *j* subscripts the salmon permit (Table 1). We define the variables $\Delta { S }_{ t }=\log\left( { S }_{ t }/{ S }_{ t-1 } \right)$ to represent the percent change in species diversity of the catch and $\Delta { D }_{ t }=\log\left( { D }_{ t }/{ D }_{ t-1 } \right)$ to represent the percent change in fishing effort (days fished) between years. Time-varying intercepts, specific to each permit (Table 1), are modeled as random and exchangeable across years, ${ B }_{ 0,j,t }\sim \mathrm{Normal}\left( { u }_{ { B }_{ 0 } },{ \sigma  }_{ { B }_{ 0 } } \right)$. The term ${ B }_{ 1,j,t }$ represents the permit-level random effect of increasing catch diversity. We also included the terms $\Delta S_{ t }\cdot { S }_{ t-1 }$, which forces the effect of diversification to be linear but allows the strength of it to vary through time. The coefficients ${ B }_{ 1,j,t }$ and ${ B }_{ 2,j,t }$ allow the interaction to vary by permit and year. We did not include main effect terms for ${ S }_{ t-1 }$, because the expectation of log-differenced revenue was assumed to be zero, regardless of prior catch diversity. The flexibility of this model allows the benefit of increasing catch diversity to be positive in some years (generalizing translates to increased revenue), negative in other years (specializing increases revenue) and neutral in other years (revenue is independent of alterations to catch composition). We constrain the random effects associated with diversification to be modeled as independent by year (not autoregressive) with shared variances across strategies, for example: ${ B }_{ 1,j,t }\sim \mathrm{Normal}\left( 0, { \sigma  }_{ { B }_{ 1 } } \right)$. The effect of changing effort (measured as percent change in days fished) ${ B }_{ 3,j }$ is varies by strategy, but we did not allow this effect to vary through time. Finally, the term $\log\left( { R }_{ i,t-1 } \right)$ is included as an offset because of non-stationarity in .

Our second model involved modeling the variability of annual revenue that individuals experience. Simultaneously modeling the variance and mean components makes the model akin to variance function regression or stochastic volatility models [@smyth1989; @western2009; @anderson_benefits]. The model for the standard deviation is $$\log\left( { \sigma  }_{ i,t } \right)={ G }_{ 0,j }+{ G }_{ 1,j,t }\cdot \Delta S_{ i,t }+{ G }_{ 2,j,t }\cdot \left( \Delta S_{ i,t }\cdot { S }_{ i,t-1 } \right) +G_{ 3,j }\cdot \Delta D_{ i,t }$$. Unlike the linear model for the mean, an offset term is not included for the variance. Further, we constrained the variance intercepts ${G}_{0,j}$ to be constant by strategy. Like with the mean model, the random effects were constrained to be independent by strategy and year, ${ G }_{ 1,j,t }\sim \mathrm{Normal}\left( 0, { \sigma  }_{ { G }_{ 1 } } \right)$, and covariates associated with days fished, $\Delta D_{ i,t }$, and species diversity $\Delta S_{ i,t }$ were included as predictors.
