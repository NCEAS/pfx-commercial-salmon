...
To quantify potential effects between hatchery runs and diveristy, we used data from the seine and gillnet fisheries in Prince William Sound and Southeast Alaska to correlate catch diversity (Figs. A3-A4) with hatchery returns of dominant species (Fig. 1). In Southeast Alaska, we correlated catch diversity with the size of the hatchery chum returns, because chum are caught by both seine and gillnet fisheries (Figs. A3-A4). For Prince William Sound, we calculated the catch diversity of the seine fishery with hatchery returns of pink salmon (Fig. A3) and the catch diversity of the gillnet fishery with hatchery returns of sockeye salmon (Fig. A4). 

To quantify relationships between hatchery production and revenue or revenue stability (variance), we used the landed catch from each region and species as a response [@amoroso2017; @hilborn2001a; @knapp2007]. Using a variance function regression model, we extendeded the approach of [@amoroso2017] to model variability in salmon catches as a function of the hatchery contribution. Within each region (Prince William Sound, Southeast, Kodiak, Alaska Peninsula), the hatchery contribution and total harvest are recorded by year for each species [@stopha2016] (Fig. 1), with Kodiak and the Alaska Peninsula grouped into a single region because of reporting. The catch of each species in each region was modeled as, $$E[{ Y }_{ s,f,t }]={ Y }_{ s,f,t-1 }+{ b }_{ s,f }{ X }_{ s,f,t }$$ where ${ Y }_{ s,f,t-1 }$ is the catch of species $s$ in region $f$ in the previous time step $t-1$, ${ b }_{ s,f }$ is a coefficient relating the contribution of hatchery fish to the mean catch, and ${ X }_{ s,f,t }$ is the change in proportion of hatchery contribution to the total catch for species *s* between times *t* and *t-1*. The hatchery-effect coefficients were modeled hierarchically, so ${ b }_{ s,f }\sim \mathrm{Normal}\left( { u }_{ b },{ \sigma  }_{ b } \right)$, where ${ u }_{ b }$ and ${ \sigma  }_{ b }$ represent the mean and variance of the random effects. Our second equation relates the change in proportion of hatchery contribution to the catch variability, $$log\left( { \sigma  }_{ s,f,t } \right) ={ g  }_{ 0,s,f }+{ g  }_{ 1,s,f }{ X }_{ s,f,t }$$ where ${ g  }_{ 0,s,f }$ represents an intercept unique to each region and species, and ${ g  }_{ 1,s,f }$ represents a region- and species-specific slope, ${ g }_{ 1,s,f }\sim \mathrm{Normal}\left( { u }_{ g },{ \sigma  }_{ g } \right)$. Combining the regression of the mean and variance, the observed catches were assumed to be normal $ln\left( { Y }_{ s,f,t } \right) \sim \mathrm{Normal}\left( E\left[ ln\left( { Y }_{ s,f,t } \right)  \right] ,{ \sigma  }_{ s,f,t } \right)$.
...
```{r fig4, fig.pos="placeHere", fig.cap="Time-varying effects of increasing the effective species diversity of catch (1/Simpson's diversity) from 1 to 2 species (presented as log ratio).", echo=FALSE}
# make yearpermit df
