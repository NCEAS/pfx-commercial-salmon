...
transformed parameters {
  ...
  for(m in 1:M) {
    pred[1,m] = 0;
    for(t in 2:N) {
      pred[t,m] = Y[t-1,m] + X[t,m]*b1[m];
      pred_sig[t,m] = exp(sigma0[m] + X[t,m]*sigma1[m]);
    }

    effect_sig[m] = log(exp(sigma0[m] + 0.33333*sigma1[m] ) / exp(sigma0[m]));
  }
}
model {
  musigma1 ~ cauchy(0,5);
  ...
  sigb1 ~ cauchy(0,5);
  for(i in 1:M) {
    sigma0[i] ~ normal(0,5); # fixed effect
